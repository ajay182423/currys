...
import { IAuditParameterForm } from '../../interfaces/settings/audit-parameter-form';


@Injectable({
  providedIn: 'root'
})
export class AuditParameterFormService {
  ...
  public getRecord(id: number): Observable<IAuditParameterForm>{
    return this.http.get<IAuditParameterForm>(this.baseUrl + id);
  }

  public getRecords(page?: number, itemsPerPage?: number) {
    let params = getPaginationHeaders(page, itemsPerPage);
    return getPaginatedResult<IAuditParameterForm[]>(this.baseUrl, params, this.http).pipe(
      tap(response => { this.isUpdating$.next(''); return response; }),
      catchError((err) => { this.isUpdating$.next(''); throw ''; })
    );
  }

  public create(obj: IAuditParameterForm) {
  ...
  public update(obj: IAuditParameterForm) {
  ...
  public GetAllAuditForms(): Observable<any>{
