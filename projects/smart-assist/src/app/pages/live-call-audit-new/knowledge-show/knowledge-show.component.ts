...
import { MapPopupComponent } from '../../live-call-audit/map-popup/map-popup.component';
import { ModalPopupService } from 'common_modules/services/modal-popup.service';

@Component({
  selector: 'app-knowledge-show',
  templateUrl: './knowledge-show.component.html',
  styleUrls: ['./knowledge-show.component.scss']
})
export class KnowledgeShowComponent implements OnInit {
  ...
  ngOnInit(): void {
      ...
      if(res != null) {
        if(res.length > 0) {
          ...
          if(this.recommendationShow == false) {
            this.detailsNotificationCount = res.filter(f => (f.entities != ' ' || f.Feedback != '' || f.action != '') && (this.convertToSeconds(f.end_time.split(':')[0],
    ...
    const addButton = document.getElementById('ka');
    if (addButton) {
      addButton.click();
    }
  ...
  viewMap(linkUrl){
    ...
    this.modalPopupService.openModalPopup({
      ...
      isNewEntry: false,
      popupPosition: 'center',
      heading: 'View Direction',
      width: '90vw',
      height: '85vh',
      popup:  MapPopupComponent
    });
  }
  entityVerified() {
