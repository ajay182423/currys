...
import { AudioDetailComponent } from './audio-details/audio-details.component';
...
@Component({
  ...
  styleUrls: ['./call-audit.component.scss'],
})
export class CallAuditComponent implements OnInit, OnDestroy {
  ...
  sentimentBoxShow:boolean = false;
  postOtherData:any={};
  editedData :any = {};
  ...
  ngOnInit(): void {
    this.helperService.currentData$.subscribe(obj => {
      ...
      if(obj) {
        ...
        console.log(this.editedData.otherProperties)
        ...
        // let abc = this.customForm[obj.index].controls['AssociatedTranscriptObj'].patchValue()
      }
    });
  ...
  GetApiTranscriptData(transactionId: string) {
    ...
    this.CallAuditService.getMlTranscript(param).subscribe((d) => {
      ...
      this.sentimentdataset = d.transcript.map(m => m.Sentiment);
        ...
        if(d.transcript.length > 0){this.sentimentBoxShow = true;}
    })
  ...
  getAudit() {
    ...
    if (!this.transactionId) {
      ...
      this.CallAuditService.getRecord(this.transactionId).subscribe((data) => {
        ...
        if(this.sectionData.transactionId === 'f366e5c42f0b1de67c5c2de0edb1d090_2' || this.sectionData.transactionId === 'f71aa7357e98d9c7031956c004ce3e2e_2'
        || this.sectionData.transactionId === 'f3b6c4684a92644ff2c8feedfc8da852_2' ||this.sectionData.transactionId === 'd35b6e8d14f5667e11de0e082eac0715'
        || this.sectionData.transactionId === 'f3b6c4684a92644ff2c8feedfc8da852_2'|| this.sectionData.transactionId === 'f594a2aca3fb20f4322cbb654936a50a_2'
        || this.sectionData.transactionId === "fdc05ff37babb18df6211be3ab0860d0_2"){
          this.scoreColumn = true;
          this.wholeColumn = false;
        }else{
          this.scoreColumn = false;
        ...
        data?.sections?.forEach((section) => {
          ...
          //declare total score var for sections
          let totalScoreOfSectionShouldBe = 0;
          let totalScoreOfSectionAchieved = 0;
          let len = section?.results?.length;
          if(section.section=='Understanding Our Customer')
            this.UnderstandingScore = section.results[0].score;
          if(section.section=='Instilling confidence')
            this.InstillingScore = section.results[0].score;
          if(section.section=='Resolution')
            this.ResolutionScore = section.results[0].score;
          if(section.section=='Informing and Actioning')
            this.InformingScore = section.results[0].score;
          if(section.section=='Concluding')
            this.ConcludingScore = section.results[0].score;
  ...
  submitAudit() {
    ...
    console.log(this.PostCallAuditObj)
