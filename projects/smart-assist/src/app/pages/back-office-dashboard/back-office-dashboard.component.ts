...
// import { Transactionservice } from '../../services/email.service';
import { AuditParameterFormService } from '../../services/settings/audit-parameter-form.service';
import { AuditParameterResultService } from '../../services/pages/audit-parameter-result.service';
import { SectionService } from '../../services/settings/section.service';
import { AuditParameterService } from '../../services/settings/audit-parameter.service';
...
export class BackOfficeDashboardComponent implements OnInit {
  ...
  // Local JsonFile
  jsonUrl :string =  environment.jsonFilesUrl + 'back-office-dashboard.json'
  jsonAgentData :string =  environment.jsonFilesUrl + 'agentData.json'

  // Server Json File
  fileUrl: string = environment.fileUrl + 'back-office-dashboard.json';
  ...
  pageSize: number = 10;
    ...
    tableHeaders: any = [
        ...
        {id: 2, name: "No. of Transactions", order: 'desc', type: 'number',  class: 'th-sort'},
        ...
        {id: 6, name: "No. of Flows", order: 'desc', type: 'number',  class: 'th-sort'}
  ...
  constructor(
    ...
    private auditParametersResultService : AuditParameterResultService,
    private sectionService : SectionService,
    private auditParameterService : AuditParameterService,
    private auditFormService : AuditParameterFormService,
  ...
  ngOnInit(): void {
       ...
       this.categorizationChart('journey');
...
// }
    ...
    fillTableRows(){
        // this.dashboardService.getTableData(this.pageNumber, this.pageSize).pipe(finalize(() => {
        this.helperService.getJSON(this.jsonUrl).subscribe(response => {
          if (response.agents.length > 0) {
            this.tableData = {
              currentPage: 1, //response.pagination.currentPage,
              itemsPerPage: 10, //response.pagination.itemsPerPage,
              totalItems:  10, //response.pagination.totalItems,
              totalPages: 1, // response.pagination.totalPages,
              pageNumbers: 1, //Array(response.pagination.totalPages).fill(0).map((x,i)=>i+1),
              tableRows: response.agents.map(m => ({
                0:  {value:m.agentName},
                1:  {value:m.agentName, link: m.agentId, class: 'primary_link', linkable : false},
                2:  {value:m.noOfCalls},
...
SelectPerformanceGraphShow(graphName:string){
  if(graphName === 'Day'){
    ...
    const canvas = <HTMLCanvasElement>document.getElementById('widget1_tab1Chart');
  ...
  if (graphName === 'Month'){
      ...
      this.helperService.getJSON(this.jsonUrl).subscribe(data => {
        ...
        const canvas = <HTMLCanvasElement>document.getElementById('widget1_tab2Chart');
        const ctx = canvas?.getContext('2d');
...
createSingleChart() {
  this.helperService.getJSON(this.jsonUrl).subscribe(data => {
    var canvas = document.getElementById('singleTabChart') as HTMLCanvasElement;
    var ctx = canvas.getContext('2d')
    var chartStatus = new Chart(ctx, {
        type: 'doughnut',
        data: {
            labels: this.widget3_tab1ChartData.body.data.map(m => {return m.name}) ,
              datasets: [{
                label: this.widget3_tab1ChartData.body.chartTitle,
                data: this.widget3_tab1ChartData.body.data.map(m => {return m.values.reduce((arr, item) => {
                      return arr + item
                    })}),
                backgroundColor: this.widget3_tab1ChartData.body.data.map(m => {return m.bg }),
              }]
        },
        options: {
            plugins:{
  ...
  if (graphName === 'IdvCompliance'){
    ...
    this.pciComplianceChartShow=false;
      this.helperService.getJSON(this.jsonUrl).subscribe(data => {
        ...
        var canvas = document.getElementById('widget2_tab1Chart') as HTMLCanvasElement;
        var ctx = canvas.getContext('2d')
        var chartStatus = new Chart(ctx, {
            type: 'doughnut',
            data: {
                labels: this.widget2_tab1ChartData.body.data.map(m => {return m.name}) ,
                  datasets: [{
                    label: this.widget2_tab1ChartData.body.chartTitle,
                    data: this.widget2_tab1ChartData.body.data.map(m => {return m.values.reduce((arr, item) => {
                          return arr + item
                        })}),
                    backgroundColor: this.widget2_tab1ChartData.body.data.map(m => {return m.bg }),
                  }]
            },
            options: {
                plugins:{
    ...
    this.pciComplianceChartShow=true;
      ...
      this.helperService.getJSON(this.jsonUrl).subscribe(data => {
        ...
        var canvas = document.getElementById('widget2_tab2Chart') as HTMLCanvasElement;
        var ctx = canvas.getContext('2d')
        var chartStatus = new Chart(ctx, {
            type: 'doughnut',
            data: {
                labels: this.widget2_tab2ChartData.body.data.map(m => {return m.name}) ,
                  datasets: [{
                    label: this.widget2_tab2ChartData.body.chartTitle,
                    data: this.widget2_tab2ChartData.body.data.map(m => {return m.values.reduce((arr, item) => {
                          return arr + item
                        })}),
                    backgroundColor: this.widget2_tab2ChartData.body.data.map(m => {return m.bg }),
                  }]
            },
            options: {
                plugins:{
      ...
      this.helperService.getJSON(this.jsonUrl).subscribe(data => {
        ...
        var canvas = document.getElementById('widget2_tab3Chart') as HTMLCanvasElement;
        var ctx = canvas.getContext('2d')
        var chartStatus = new Chart(ctx, {
            type: 'doughnut',
            data: {
                labels: this.widget2_tab3ChartData.body.data.map(m => {return m.name}) ,
                  datasets: [{
                    label: this.widget2_tab3ChartData.body.chartTitle,
                    data: this.widget2_tab3ChartData.body.data.map(m => {return m.values.reduce((arr, item) => {
                          return arr + item
                        })}),
                    backgroundColor: this.widget2_tab3ChartData.body.data.map(m => {return m.bg }),
                  }]
            },
            options: {
                plugins:{
      ...
      this.helperService.getJSON(this.jsonUrl).subscribe(data => {
        ...
        var canvas = document.getElementById('widget2_tab4Chart') as HTMLCanvasElement;
        var ctx = canvas.getContext('2d')
        var chartStatus = new Chart(ctx, {
            type: 'doughnut',
            data: {
                labels: this.widget2_tab4ChartData.body.data.map(m => {return m.name}) ,
                  datasets: [{
                    label: this.widget2_tab4ChartData.body.chartTitle,
                    data: this.widget2_tab4ChartData.body.data.map(m => {return m.values.reduce((arr, item) => {
                          return arr + item
                        })}),
                    backgroundColor: this.widget2_tab4ChartData.body.data.map(m => {return m.bg }),
                  }]
            },
            options: {
                plugins:{
...
categorizationChart(graphName:string){
  if (graphName === 'journey'){
      ...
      this.helperService.getJSON(this.jsonUrl).subscribe(data => {
        ...
        var canvas = document.getElementById('widget3_tab1Chart') as HTMLCanvasElement;
        var ctx = canvas.getContext('2d')
        var chartStatus = new Chart(ctx, {
            type: 'doughnut',
            data: {
      ...
      this.helperService.getJSON(this.jsonUrl).subscribe(data => {
        ...
        var canvas = document.getElementById('widget3_tab2Chart') as HTMLCanvasElement;
        var ctx = canvas.getContext('2d')
        var chartStatus = new Chart(ctx, {
            type: 'doughnut',
            data: {
                labels: this.widget3_tab2ChartData.body.data.map(m => {return m.name}) ,
                  datasets: [{
                    label: this.widget3_tab2ChartData.body.chartTitle,
                    data: this.widget3_tab2ChartData.body.data.map(m => {return m.values.reduce((arr, item) => {
                          return arr + item
                        })}),
                    backgroundColor: this.widget3_tab2ChartData.body.data.map(m => {return m.bg }),
                  }]
            },
            options: {
                plugins:{
      ...
      this.helperService.getJSON(this.jsonUrl).subscribe(data => {
        ...
        var canvas = document.getElementById('widget3_tab3Chart') as HTMLCanvasElement;
        var ctx = canvas.getContext('2d')
        var chartStatus = new Chart(ctx, {
            type: 'doughnut',
            data: {
                labels: this.widget3_tab3ChartData.body.data.map(m => {return m.name}) ,
                  datasets: [{
                    label: this.widget3_tab3ChartData.body.chartTitle,
                    data: this.widget3_tab3ChartData.body.data.map(m => {return m.values.reduce((arr, item) => {
                          return arr + item
                        })}),
                    backgroundColor: this.widget3_tab3ChartData.body.data.map(m => {return m.bg }),
                  }]
            },
            options: {
                plugins:{
    ...
    if (this.timePeriod == "Today") {
        this.dashboardService.refreshNeeded$.subscribe(() => {
            this.dashboardService.getStatsbyMailbox(mailboxAd).subscribe(data => {
                ...
                if (mailboxAd == "All") {
                    this.dashboardService.getStatsAvgTATToday(mailboxAd).subscribe(data => this.avgTAT = data);
                    this.dashboardService.getSentimentDetails("Today", mailboxAd).subscribe(data => this.setimentData = data);
                }
                else {
                    this.dashboardService.getStatsAvgTATToday(mailboxAd + '@exlservice.com').subscribe(data => this.avgTAT = data);
                    this.dashboardService.getSentimentDetails("Today", mailboxAd + '@exlservice.com').subscribe(data => this.setimentData = data);
                }
        ...
        this.dashboardService.refreshNeeded$.subscribe(() => {
            this.dashboardService.getStatsbyMailboxTotal(mailboxAd).subscribe(data => {
                this.pending = data.pendingCount;
                this.completed = data.completedCount;
                this.total = data.totalCount;
                this.hold = data.holdCount;
                ...
                this.StatusChart(this.total, this.completed, this.pending);
                if (mailboxAd == "All") {
                    this.dashboardService.getStatsAvgTATTotal(mailboxAd).subscribe(data => this.avgTAT = data);
                    this.dashboardService.getSentimentDetails("Total", mailboxAd).subscribe(data => this.setimentData = data);
                }
                else {
                    this.dashboardService.getStatsAvgTATTotal(mailboxAd + '@exlservice.com').subscribe(data => this.avgTAT = data);
                    this.dashboardService.getSentimentDetails("Total", mailboxAd + '@exlservice.com').subscribe(data => this.setimentData = data);
                }
            });
        });
        this.dashboardService.getStatsbyMailboxTotal(mailboxAd).subscribe(data => {
        ...
        this.dashboardService.refreshNeeded$.subscribe(() => {
            this.dashboardService.getStatsbyMailboxMonthly(mailboxAd).subscribe(data => {
                this.pending = data.pendingCount;
                this.completed = data.completedCount;
                this.total = data.totalCount;
                this.hold = data.holdCount;
                ...
                this.StatusChart(this.total, this.completed, this.pending);
                if (mailboxAd == "All") {
                    this.dashboardService.getStatsAvgTATMonth(mailboxAd).subscribe(data => this.avgTAT = data);
                    this.dashboardService.getSentimentDetails("Monthly", mailboxAd).subscribe(data => this.setimentData = data);
                }
                else {
                    this.dashboardService.getStatsAvgTATMonth(mailboxAd + '@exlservice.com').subscribe(data => this.avgTAT = data);
                    this.dashboardService.getSentimentDetails("Monthly", mailboxAd + '@exlservice.com').subscribe(data => this.setimentData = data);
                }
            });
        });
        this.dashboardService.getStatsbyMailboxMonthly(mailboxAd).subscribe(data => {
        ...
        this.dashboardService.refreshNeeded$.subscribe(() => {
            this.dashboardService.getStatsbyMailboxWeekly(mailboxAd).subscribe(data => {
                this.pending = data.pendingCount;
                this.completed = data.completedCount;
                this.total = data.totalCount;
                this.hold = data.holdCount;
                ...
                this.StatusChart(this.total, this.completed, this.pending);
                if (mailboxAd == "All") {
                    this.dashboardService.getStatsAvgTATWeek(mailboxAd).subscribe(data => this.avgTAT = data);
                    this.dashboardService.getSentimentDetails("Weekly", mailboxAd).subscribe(data => this.setimentData = data);
                }
                else {
                    this.dashboardService.getStatsAvgTATWeek(mailboxAd + '@exlservice.com').subscribe(data => this.avgTAT = data);
                    this.dashboardService.getSentimentDetails("Weekly", mailboxAd + '@exlservice.com').subscribe(data => this.setimentData = data);
                }
            });
        });
        this.dashboardService.getStatsbyMailboxWeekly(mailboxAd).subscribe(data => {
...
categorizationJourneyChart(labels:any, data :any){
    var canvas = document.getElementById('categorizationJourneyChart') as HTMLCanvasElement;
    var ctx = canvas.getContext('2d')
    var chartStatus = new Chart(ctx, {
        type: 'doughnut',
        data: {
...
categorizationSubJourneyChart(labels:any, data :any){
    var canvas = document.getElementById('categorizationSubJourneyChart') as HTMLCanvasElement;
    var ctx = canvas.getContext('2d')
    var chartStatus = new Chart(ctx, {
        type: 'doughnut',
        data: {
...
categorizationCustomerIntentChart(labels:any, data :any){
  var canvas = document.getElementById('categorizationCustomerIntentChart') as HTMLCanvasElement;
  var ctx = canvas.getContext('2d')
  var chartStatus = new Chart(ctx, {
      type: 'doughnut',
      data: {
